...
const Vault = props => {
  ...
  const originalVaultToken = vault.token;
  _.merge(vault, vaultContractData);
  // vault.token = originalVaultToken;
  const {
    tokenAddress,
    tokenSymbolAlias,
    decimals,
  ...
  console.log('vfff', vault);
  const tokenContractAddress = want;
  ...
  const vaultName = vaultDisplayName || name;
